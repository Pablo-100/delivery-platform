<div class="min-h-screen bg-gray-50">
    <!-- Header -->
    <div class="bg-gradient-to-r from-purple-900 via-pink-900 to-purple-900 text-white">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            <div class="flex items-center justify-between">
                <div>
                    <div class="flex items-center mb-2">
                        <a href="{{ route('super-admin.dashboard') }}" class="text-purple-200 hover:text-white mr-3">
                            ← Retour
                        </a>
                    </div>
                    <h1 class="text-3xl font-bold flex items-center">
                        <span class="mr-3">👷</span>
                        Historique: {{ $livreur->nom }} {{ $livreur->prenom }}
                    </h1>
                    <p class="mt-2 text-purple-200">{{ $livreur->email }}</p>
                </div>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4 mb-8">
            <div class="bg-white rounded-xl p-5 shadow-sm">
                ...
                <p class="text-3xl font-bold text-purple-600">{{ $stats['total_tournees'] }}</p>
            </div>
            <div class="bg-white rounded-xl p-5 shadow-sm">
            ...
            <div class="bg-white rounded-xl p-5 shadow-sm">
                <p class="text-gray-500 text-sm">Camions Différents</p>
                <p class="text-3xl font-bold text-indigo-600">{{ $stats['camions_differents'] }}</p>
            </div>
            <div class="bg-white rounded-xl p-5 shadow-sm">
        ...
        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
            <div class="p-6 border-b border-gray-100">
                ...
                <p class="text-sm text-gray-500 mt-1">Tous les camions conduits et livraisons effectuées</p>
            </div>
                ...
                @forelse($historiques as $historique)
                    <div class="mb-6 pb-6 border-b border-gray-100 last:border-0">
                            ...
                            <div class="flex items-center">
                                ...
                                <div>
                                    <h4 class="text-lg font-bold text-gray-900">
                                        🚛 {{ $historique->camion?->immatriculation ?? 'Camion inconnu' }}
                                        <span class="text-sm font-normal text-gray-500 ml-2">({{ $historique->camion?->modele ?? 'N/A' }})</span>
                                    </h4>
                                    <p class="text-xs text-gray-500">
                                        Tournée: <span class="font-mono">{{ $historique->tournee_code ?? 'N/A' }}</span>
                                        @if($historique->camion?->admin)
                                            | Propriétaire: {{ $historique->camion->admin->nom }} {{ $historique->camion->admin->prenom }}
                                        @endif
                                    </p>
                                </div>
                            </div>
                        ...
                        @if($historique->depots_visites && count($historique->depots_visites) > 0)
                            <div class="mb-4">
                                <p class="text-sm font-medium text-gray-700 mb-2">📍 Destinations visitées:</p>
                        ...
                        @if($produits->count() > 0)
                            <div>
                                <p class="text-sm font-medium text-gray-700 mb-2">📦 Produits transportés ({{ $produits->count() }}):</p>
                                <div class="flex flex-wrap gap-2">
                ...
                <div class="sticky top-0 bg-gradient-to-r from-purple-600 to-pink-600 text-white p-6 rounded-t-2xl">
                    <div class="flex items-center justify-between">
                        <div>
                            ...
                            <p class="text-purple-100 text-sm mt-1">{{ $selectedHistorique->tournee_code ?? 'N/A' }}</p>
                        </div>
                ...
                <div class="p-6">
                    ...
                    <div class="grid grid-cols-2 gap-4 mb-6">
                        <div class="bg-purple-50 p-4 rounded-lg">
                            <p class="text-xs text-purple-600 mb-1">🚛 Camion</p>
                            <p class="font-bold text-gray-900">{{ $selectedHistorique->camion?->immatriculation }}</p>
                            <p class="text-sm text-gray-600">{{ $selectedHistorique->camion?->modele }}</p>
                        </div>
                        <div class="bg-blue-50 p-4 rounded-lg">
                            <p class="text-xs text-blue-600 mb-1">👷 Livreur</p>
                            <p class="font-bold text-gray-900">{{ $livreur->nom }} {{ $livreur->prenom }}</p>
                            <p class="text-sm text-gray-600">{{ $livreur->email }}</p>
                        </div>
                    </div>
                    ...
                    @if($destinations->count() > 0)
                        <div class="mb-6">
                            <h4 class="font-bold text-gray-900 mb-3">📍 Destinations avec détails</h4>
                            <div class="space-y-2">
                    ...
                    @if($detailProduits->count() > 0)
                        <div class="mb-6">
                            <h4 class="font-bold text-gray-900 mb-3">📦 Tous les produits ({{ $detailProduits->count() }})</h4>
